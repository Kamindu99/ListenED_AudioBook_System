...
const Login = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === "c") {
        ...
        const token1 = localStorage.setItem("userId", "9");
  ...
  const getRecommndbooks = (user_History, study_Area) => {
    try {
      axios
        .post("https://listened.onrender.com/parternimage/", {
          image: user_History,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://listened.onrender.com/user/login/",
        {
          password: password,
        }
      );
  ...
  const handleSubmit2 = async (imagesrc) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/api/login/face",
        {
          face: imagesrc,
        }
      );
  ...
  const startCamera = () => {
    axios
      .get("http://127.0.0.1:5000/start_camera")
      .then((response) => {
  ...
  return (
    <div>
      {/* <Navbar /> */}
      <button className="abc123" onClick={startCamera}>
        Start Camera
      </button>

      <div className="login__container">
        <h2>ඇතුලු වන්න </h2>
        <button className="abc123" onClick={startListeningOnClick}>
          Start
        </button>
        <div>
        ...
        {renderWebcam ? (
          <div className="container">
            {timer0 && <div className="timer-container">{timer}</div>}
            <Webcam
              audio={false}
              height={200}
              ref={webcamRef}
